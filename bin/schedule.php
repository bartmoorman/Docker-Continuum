...
while (true) {
  ...
  foreach ($continuum->getObjects('monitors') as $monitor) {
    if (!$monitor['disabled'] && !$continuum->memcacheConn->get(sprintf('lastRun-%u', $monitor['monitor_id']))) {
      ...
      foreach ($continuum->getRandomEndpoints($monitor['endpoints']) as $endpoint) {
        switch ($pid = pcntl_fork()) {
          case -1:
            echo date('Y-m-d H:i:s') . " - could not fork (monitor_id: {$monitor['monitor_id']}, endpoint_id: {$endpoint['endpoint_id']})" . PHP_EOL;
            break;
          case 0:
